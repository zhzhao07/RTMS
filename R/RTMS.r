...
RTMS<- function(x,y,rp,method="3S",CS=NULL,mc=1)
{
  ...
  if(method=="XM"){
    ...
    d=cbind(R1,R2,as.numeric(R30),S1,S2,S3,F1)
    colnames(XR)=c("DC-SIS","SIRS","ISIS","ARM","AIC","BIC","RF")
    colnames(d)=c("DC-SIS","SIRS","ISIS","ARM","AIC","BIC","RF")
    d=d[,CS];
    d2=sort((rowSums(d)-rowSums(as.matrix((t(apply(d,1,function(z) sort(z, decreasing = TRUE))))[,1:mc])))/(ncol(d)-mc))
    ...
    D=d23
    b7=D  
    object=list(RTMSXM=b7,DCSIS=b1,SIRS=b2,ISIS=b3,ARM=b4,AIC=b5,BIC=b6,RF=b71)
  }
